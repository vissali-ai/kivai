...
export default async function NewsAgentPage() {
  ...
  return <main>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-2">
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><Rss className="size-4 text-primary" />Fontes monitoradas</CardTitle></CardHeader><CardContent><div className="space-y-3">{sources.map((source) => <div key={source.id} className="flex items-center justify-between gap-3 border-b border-white/5 pb-3"><div><p className="text-sm font-medium">{source.name}</p><p className="mt-1 max-w-md truncate text-xs text-muted-foreground">{source.feedUrl}</p></div><Badge variant="outline">{source.enabled ? "Ativa" : "Pausada"}</Badge></div>)}{!sources.length ? <p className="text-sm text-muted-foreground">Nenhuma fonte disponível.</p> : null}</div></CardContent></Card>
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><Clock3 className="size-4 text-primary" />Últimas execuções</CardTitle></CardHeader><CardContent><div className="space-y-3">{runs.map((run) => <div key={run.id} className="border-b border-white/5 pb-3"><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium">{date.format(new Date(run.startedAt))}</p><Badge variant="outline">{run.status === "completed" ? "Concluída" : run.status === "failed" ? "Falhou" : "Executando"}</Badge></div><p className="mt-1 text-xs text-muted-foreground">{run.draftsCreated} rascunho(s) · {run.itemsFound} item(ns) encontrado(s) · {run.itemsSkipped} ignorado(s)</p>{run.error ? <p className="mt-1 text-xs text-red-300">{run.error}</p> : null}</div>)}{!runs.length ? <p className="text-sm text-muted-foreground">O agente ainda não foi executado.</p> : null}</div></CardContent></Card>
    </div>
  </main>;
}
