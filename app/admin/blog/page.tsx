...
import { editorialFitFromScore } from "@/lib/news-agent/candidate-ranking";
...
export default async function BlogAdmin({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; origin?: string; category?: string; page?: string }> }) {
  ...
  const [allPosts, posts, categories] = await Promise.all([
    listAllPosts(),
    listPosts({ query: params.q, status, origin, categoryId: params.category, page: visiblePage, pageSize: 10 }),
    listCategories(),
  ]);
  const stats = [
  ...
  const totalPages = Math.max(1, Math.ceil(matchingPosts.length / 10));
  const pageHref = (page: number) => { const query = new URLSearchParams({ status, page: String(page) }); if (params.q) query.set("q", params.q); if (params.category) query.set("category", params.category); if (origin !== "all") query.set("origin", origin); return `/admin/blog?${query.toString()}`; };
  return <main>
    ...
    <Card className="mt-6"><CardHeader className="flex-row flex-wrap items-center justify-between gap-3"><CardTitle>Matérias</CardTitle><NewsAgentPanel canRun={isBlogDatabaseConfigured()} /></CardHeader><CardContent>
      <nav aria-label="Status das matérias" className="mb-4 flex flex-wrap gap-2 border-b border-white/10 pb-3">{statusTabs.map((tab) => <Link key={tab.value} href={statusHref(tab.value)} className={`border px-3 py-2 text-xs font-medium transition ${status === tab.value ? "border-primary/40 bg-primary/10 text-primary" : "border-white/10 text-muted-foreground hover:border-primary/30 hover:text-foreground"}`}>{tab.label}<span className="ml-2 text-[10px] opacity-70">{tab.count}</span></Link>)}</nav>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{posts.map((post) => {
        const titleUrl = post.sourceUrl || (post.status === "published" ? `/blog/${post.slug}` : `/admin/blog/${post.id}/preview`);
        const fit = editorialFitFromScore(post.relevanceScore);
        return <article key={post.id} className="min-w-0 rounded-lg border border-white/10 bg-background p-4"><Link href={titleUrl} target="_blank" rel="noopener noreferrer" className="line-clamp-3 break-words text-sm font-medium leading-snug hover:text-primary hover:underline">{post.title}</Link><div className="mt-2 flex flex-wrap items-center gap-2"><p className="text-xs text-muted-foreground">{post.category?.name ?? "Sem categoria"}</p><Badge variant="outline">{statusLabels[post.status]}</Badge>{post.origin === "rss-agent" ? <Badge variant="outline" title={fit.detail} className={fit.key === "aligned" ? "border-emerald-500/30 text-emerald-300" : fit.key === "review" ? "border-amber-500/30 text-amber-300" : "text-muted-foreground"}>{fit.label}</Badge> : null}</div><p className="mt-2 text-[11px] text-muted-foreground">Atualizada em {formatDate(post.updatedAt)}</p><div className="mt-2 flex min-h-8 items-center gap-2">{post.needsCover ? <p className="text-[11px] text-amber-300">Aguardando foto</p> : null}<PostActions id={post.id} slug={post.slug} status={post.status} mobileCompact /></div></article>;
      })}</div>
      <div className="hidden"><table className="w-full text-left text-xs"><thead className="border-b border-white/10 text-muted-foreground"><tr>{["Título", "Categoria", "Status", "Criação", "Atualização", "Publicação", "Ações"].map((item) => <th key={item} className="px-3 py-3 font-medium">{item}</th>)}</tr></thead><tbody>{posts.map((post) => {
        const titleUrl = post.sourceUrl || (post.status === "published" ? `/blog/${post.slug}` : `/admin/blog/${post.id}/preview`);
        const fit = editorialFitFromScore(post.relevanceScore);
        return <tr key={post.id} className="border-b border-white/5"><td className="max-w-xs px-3 py-3 font-medium"><div className="flex items-start gap-2"><Link href={titleUrl} target="_blank" rel="noopener noreferrer" className="hover:text-primary hover:underline">{post.title}</Link>{post.origin === "rss-agent" ? <Badge variant="outline" className="shrink-0 border-primary/30 text-primary">RSS</Badge> : null}</div>{post.origin === "rss-agent" ? <Badge variant="outline" title={fit.detail} className={`mt-1 ${fit.key === "aligned" ? "border-emerald-500/30 text-emerald-300" : fit.key === "review" ? "border-amber-500/30 text-amber-300" : "text-muted-foreground"}`}>{fit.label}</Badge> : null}{post.needsCover ? <p className="mt-1 text-[11px] font-normal text-amber-300">Aguardando foto</p> : null}</td><td className="px-3 py-3">{post.category?.name ?? "Sem categoria"}</td><td className="px-3 py-3"><Badge variant="outline">{statusLabels[post.status]}</Badge></td><td className="px-3 py-3 text-muted-foreground">{formatDate(post.createdAt)}</td><td className="px-3 py-3 text-muted-foreground">{formatDate(post.updatedAt)}</td><td className="px-3 py-3 text-muted-foreground">{formatDate(post.publishedAt)}</td><td className="px-3 py-3"><PostActions id={post.id} slug={post.slug} status={post.status} /></td></tr>;
      ...
      {posts.length ? <nav aria-label="Paginação das matérias" className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-4"><p className="text-xs text-muted-foreground">Página {Math.min(visiblePage, totalPages)} de {totalPages} · {matchingPosts.length} matéria(s)</p><div className="flex gap-2"><Button asChild={visiblePage > 1} variant="outline" disabled={visiblePage <= 1}>{visiblePage > 1 ? <Link href={pageHref(visiblePage - 1)}>Anterior</Link> : <span>Anterior</span>}</Button><Button asChild={visiblePage < totalPages} variant="outline" disabled={visiblePage >= totalPages}>{visiblePage < totalPages ? <Link href={pageHref(visiblePage + 1)}>Próxima</Link> : <span>Próxima</span>}</Button></div></nav> : null}
    </CardContent></Card>
  </main>;
}
