  ...
  return <main>
    ...
    <Card className="mt-6"><CardHeader className="flex-row flex-wrap items-center justify-between gap-3"><CardTitle>Matérias</CardTitle><NewsAgentPanel canRun={isBlogDatabaseConfigured()} /></CardHeader><CardContent>
      ...
      <div className="divide-y divide-white/5 md:hidden">{posts.map((post) => {
        ...
        return <article key={post.id} className="py-3"><Link href={titleUrl} target="_blank" rel="noopener noreferrer" className="line-clamp-2 text-sm font-medium leading-snug hover:text-primary hover:underline">{post.title}</Link><div className="mt-1 flex flex-wrap items-center gap-2"><p className="text-xs text-muted-foreground">{post.category?.name ?? "Sem categoria"}</p>{post.origin === "rss-agent" ? <Badge variant="outline" title={fit.detail} className={fit.key === "aligned" ? "border-emerald-500/30 text-emerald-300" : fit.key === "review" ? "border-amber-500/30 text-amber-300" : "text-muted-foreground"}>{fit.label}</Badge> : null}</div><div className="mt-1.5 flex min-h-8 items-center gap-2">{post.needsCover ? <p className="text-[11px] text-amber-300">Aguardando foto</p> : null}<PostActions id={post.id} slug={post.slug} status={post.status} mobileCompact /></div></article>;
      })}</div>
