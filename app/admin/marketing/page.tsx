...
export default async function MarketingPage({ searchParams }: { searchParams: Promise<{ segment?: string }> }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="border border-primary/25 bg-primary/[0.035] p-5 sm:p-6">
      <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Onboarding automático</p><h2 className="mt-1 text-xl font-semibold">E-mails automáticos da conta</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-muted-foreground">Estes modelos são disparados automaticamente quando ocorre o evento correspondente. Cada tentativa também aparece na Fila de comunicações com sucesso ou falha. “sent” significa aceito pelo Resend. Pasta Spam não pode ser detectada diretamente; quando integrarmos o webhook, reclamações de spam poderão ser registradas como “complained”.</p></div><Mail className="size-6 text-primary" /></div>
