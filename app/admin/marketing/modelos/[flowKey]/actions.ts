...
function validPair(label: string, url: string) {
  if (!label && !url) return true;
  return Boolean(label && /^https:\/\//i.test(url));
}
...
export async function saveMarketingTemplate(flowKey: string, formData: FormData) {
  ...
  const ctaUrl = text(formData, "ctaUrl", 800);
  const secondaryCtaLabel = text(formData, "secondaryCtaLabel", 80);
  const secondaryCtaUrl = text(formData, "secondaryCtaUrl", 800);
  if (!subject || !message) throw new Error("Assunto e conteúdo são obrigatórios.");
  ...
  await supabaseRest(`customer_marketing_templates?flow_key=eq.${encodeURIComponent(flowKey)}`, {
    method: "PATCH",
    body: JSON.stringify({ subject, message, cta_label: ctaLabel || null, cta_url: ctaUrl || null, secondary_cta_label: secondaryCtaLabel || null, secondary_cta_url: secondaryCtaUrl || null, enabled: formData.get("enabled") === "on", updated_at: new Date().toISOString() }),
  });
