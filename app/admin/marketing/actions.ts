...
export async function sendMarketingTemplateNow(formData: FormData) {
  ...
  const communication = await supabaseRest<Array<{ id: string }>>("customer_communications", {
    ...
    body: JSON.stringify({
      user_id: userId,
      event_key: `template_${flowKey}_${Date.now()}`,
      channel: "email",
      status: "ready",
      ...
      scheduled_for: now,
      metadata: { source: "admin_suggestion", flow_key: flowKey, template_version: template.updated_at },
    }),
  });
  ...
  await supabaseRest("customer_marketing_events", {
    ...
    body: JSON.stringify({
      user_id: userId,
      event_type: result.status === "sent" ? "suggested_email_sent" : result.status === "canceled" ? "suggested_email_canceled" : "suggested_email_failed",
      description: result.status === "sent" ? `E-mail enviado pelo Resend: ${template.subject}` : result.status === "canceled" ? "Envio cancelado porque o usuário não recebe marketing." : `Falha no envio do modelo ${flowKey}.`,
      metadata: { flow_key: flowKey, delivery_status: result.status },
    }),
  });
...
export async function removeCustomerFromMarketingFlow(formData: FormData) {
  ...
  const now = new Date().toISOString();
  await supabaseRest(`customer_marketing_flow_enrollments?id=eq.${encodeURIComponent(enrollmentId)}&user_id=eq.${encodeURIComponent(userId)}`, {
    method: "PATCH",
    body: JSON.stringify({ status: "cancelled", updated_at: now, metadata: { source: "admin_manual", removed_at: now } }),
  });
...
export async function saveMarketingTemplate(formData: FormData) {
  ...
  const now = new Date().toISOString();
  await supabaseRest(`customer_marketing_templates?flow_key=eq.${encodeURIComponent(flowKey)}`, {
    method: "PATCH",
    body: JSON.stringify({ title, subject, description, message, cta_label: ctaLabel, cta_url: ctaUrl, enabled, updated_at: now }),
  });
  revalidatePath("/admin/marketing");
