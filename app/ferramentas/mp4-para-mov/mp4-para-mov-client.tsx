...
import { MP4_MOV_ACCEPT, MP4_MOV_MAX_FILE_SIZE, MP4_MOV_MAX_FILE_SIZE_LABEL, type Mp4MovFps, type Mp4MovQuality, type Mp4MovResolution } from "./config";
...
const fallbackError = "Não foi possível converter este vídeo para MOV. Tente novamente com outro arquivo.";
...
function codecLabel(codec: string | null) {
  ...
  return ({ h264: "H.264", hevc: "HEVC/H.265", aac: "AAC", mpeg4: "MPEG-4", mp3: "MP3", alac: "ALAC" } as Record<string, string>)[codec] || codec.toUpperCase();
}
...
export default function Mp4ParaMovClient() {
  ...
  const [quality, setQuality] = useState<Mp4MovQuality>("auto");
  const [resolution, setResolution] = useState<Mp4MovResolution>("original");
  const [fps, setFps] = useState<Mp4MovFps>("original");
  ...
  function clearSelection() {
    inspectController.current?.abort(); convertController.current?.abort(); clearResult(); setFile(null); setMetadata(null); setStatus("idle"); setActivity(null); setStage(""); setError("");
  }

  async function select(files: File[]) {
    const next = files[0];
    if (!next || activity === "convert") return;
    inspectController.current?.abort(); clearResult(); setMetadata(null); setError("");
    const extension = next.name.split(".").pop()?.toLowerCase();
    if (!next.size) { setFile(null); setError("Selecione um arquivo MP4 válido."); setStatus("error"); return; }
    if (next.size > MP4_MOV_MAX_FILE_SIZE) { setFile(null); setError("O vídeo ultrapassa o limite permitido."); setStatus("error"); return; }
    if (extension !== "mp4" || (next.type && !["video/mp4", "application/mp4", "application/octet-stream"].includes(next.type))) { setFile(null); setError("Selecione um arquivo MP4 válido."); setStatus("error"); return; }
    ...
    try {
      ...
      const response = await fetch(`${backendUrl()}/video/mp4-to-mov/inspect`, { method: "POST", body: form, cache: "no-store", signal: controller.signal });
    ...
    } finally { if (inspectController.current === controller) { inspectController.current = null; setActivity(null); } }
  }

  async function convert() {
    if (!file || !metadata || status === "processing") return;
    const controller = new AbortController(); convertController.current = controller;
    setError(""); setStatus("processing"); setActivity("convert"); setStage(willRemux ? "Convertendo para MOV sem recodificação" : "Recodificando e gerando o MOV");
    try {
      ...
      const response = await fetch(`${backendUrl()}/video/mp4-to-mov`, { method: "POST", body: form, cache: "no-store", signal: controller.signal });
      if (!response.ok) throw new Error(await responseMessage(response, fallbackError));
      const blob = await response.blob();
      if (!blob.size || !blob.type.includes("quicktime")) throw new Error(fallbackError);
      clearResult();
      ...
      setStage("");
    } finally { if (convertController.current === controller) { convertController.current = null; setActivity(null); } }
  }
  ...
  return <ToolPageShell title="Converter MP4 para MOV" description="Transforme vídeos MP4 em MOV para facilitar edição, compatibilidade e uso em diferentes programas." categoryName="Vídeos" categoryHref="/ferramentas/videos" breadcrumbRootName="Início" breadcrumbRootHref="/" processingMode="server" privacyMessage="O vídeo é enviado por conexão segura ao serviço de conversão e removido após o processamento.">
    <Card className="mx-auto max-w-5xl overflow-hidden"><CardHeader><CardTitle>Converter MP4 para MOV</CardTitle><CardDescription>Um vídeo MP4 por vez, até {MP4_MOV_MAX_FILE_SIZE_LABEL}. Resolução, proporção e duração são preservadas por padrão.</CardDescription></CardHeader><CardContent className="space-y-6">
      ...
      {metadata && !result && <><fieldset className="rounded-lg border border-border p-4"><legend className="px-2 font-medium">Configurações do MOV</legend><div className="mt-2 grid gap-4 md:grid-cols-3"><Select label="Qualidade" value={quality} onChange={(value) => setQuality(value as Mp4MovQuality)} disabled={status === "processing"} options={[["auto", "Automática"], ["high", "Alta qualidade"], ["small", "Arquivo menor"]]} /><Select label="Resolução" value={resolution} onChange={(value) => setResolution(value as Mp4MovResolution)} disabled={status === "processing"} options={[["original", "Manter original"], ["2160", "2160p"], ["1080", "1080p"], ["720", "720p"], ["480", "480p"]]} /><Select label="FPS" value={fps} onChange={(value) => setFps(value as Mp4MovFps)} disabled={status === "processing"} options={[["original", "Manter original"], ["60", "60 FPS"], ["30", "30 FPS"], ["24", "24 FPS"]]} /></div><p className="mt-3 text-sm text-muted-foreground">A ferramenta nunca aumenta automaticamente a resolução ou o FPS do arquivo.</p></fieldset>
        <div role="status" className={`border p-4 text-sm ${willRemux ? "border-primary/30 bg-primary/5" : "border-border bg-muted/20"}`}><p className="font-medium">{willRemux ? "Conversão rápida disponível" : "Recodificação necessária"}</p><p className="mt-1 leading-6 text-muted-foreground">{willRemux ? "O vídeo pode ser convertido para MOV sem recodificação, preservando a qualidade original." : "Este arquivo ou as opções selecionadas exigem recodificação para gerar um MOV compatível. O processamento pode levar mais tempo."}</p></div>
        <ToolErrorMessage message={error} /><ToolActionBar><Button size="lg" onClick={() => void convert()} disabled={status === "processing"}><FileVideo2 className="size-4" />Converter para MOV</Button>{activity === "convert" && <Button variant="outline" onClick={cancel}><X className="size-4" />Cancelar conversão</Button>}</ToolActionBar></>}
      {!file && status === "error" && <ToolErrorMessage message={error} />}
      {result && <ToolResultCard title="MOV pronto para baixar" description={`${result.name} · ${formatDuration(result.duration)} · ${result.width} × ${result.height}px · ${formatFileSize(result.blob.size)}`} preview={!previewFailed ? <video src={result.url} controls preload="metadata" className="max-h-[32rem] w-full bg-black" aria-label="Prévia do MOV convertido" onError={() => setPreviewFailed(true)} /> : <p className="border border-border bg-muted/20 p-4 text-sm text-muted-foreground">O arquivo MOV foi gerado com sucesso. Alguns navegadores podem não reproduzir este formato diretamente.</p>} details={<div className="grid gap-4 sm:grid-cols-2"><ResultSummary title="Arquivo original" lines={[`MP4 · ${formatFileSize(result.originalSize)}`, `${result.originalWidth} × ${result.originalHeight} · ${formatDuration(result.originalDuration)}`]} /><ResultSummary title="Arquivo convertido" lines={[`MOV · ${formatFileSize(result.blob.size)}`, `${result.width} × ${result.height} · ${formatDuration(result.duration)}`, `${codecLabel(result.videoCodec)} · ${result.audioCodec ? codecLabel(result.audioCodec) : "Sem áudio"}`]} /><p className="sm:col-span-2 text-sm font-medium text-primary">{result.strategy === "remux" ? "Conversão sem perda de qualidade por remux" : "Conversão com recodificação"}</p></div>} actions={<><Button onClick={download}><Download className="size-4" />Baixar MOV</Button><Button variant="outline" onClick={clearSelection}><RotateCcw className="size-4" />Converter outro vídeo</Button></>} />}
    </CardContent></Card>
  </ToolPageShell>;
}
...
function Select({ label, value, onChange, disabled, options }: { label: string; value: string; onChange: (value: string) => void; disabled: boolean; options: readonly (readonly [string, string])[] }) { const id = `mp4-mov-${label.toLowerCase()}`; return <label htmlFor={id} className="grid gap-2 text-sm font-medium">{label}<select id={id} value={value} onChange={(event) => onChange(event.target.value)} disabled={disabled} className="min-h-11 rounded-md border border-input bg-background px-3">{options.map(([optionValue, optionLabel]) => <option key={optionValue} value={optionValue}>{optionLabel}</option>)}</select></label>; }
