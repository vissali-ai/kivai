"use client";

import Link from "next/link";
import { useState } from "react";
import {
  ArrowLeft,
  Download,
  FileImage,
  FileText,
  RotateCcw,
  Upload,
} from "lucide-react";
import JSZip from "jszip";

import { AdSlot } from "@/components/ads/AdSlot";
import { Button } from "@/components/ui/button";
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";

type OutputFormat = "png" | "jpg";

export default function PdfParaImagensClient() {
  const [file, setFile] = useState<File | null>(null);
  const [format, setFormat] = useState<OutputFormat>("png");
  const [totalPages, setTotalPages] = useState<number | null>(null);
  const [loading, setLoading] = useState(false);
  const [images, setImages] = useState<string[]>([]);
  const [error, setError] = useState<string | null>(null);

  function changeFormat(nextFormat: OutputFormat) {
    if (nextFormat === format) return;
    setFormat(nextFormat);
    setImages([]);
    setError(null);
  }

  async function handleFile(event: React.ChangeEvent<HTMLInputElement>) {
    if (!event.target.files?.length) return;

    const selectedFile = event.target.files[0];
    const looksLikePdf =
      selectedFile.type === "application/pdf" || /\.pdf$/i.test(selectedFile.name);

    setImages([]);
    setTotalPages(null);
    setError(null);
    setFile(null);

    if (!looksLikePdf) {
      setError("Selecione um arquivo PDF válido.");
      event.target.value = "";
      return;
    }

    try {
      const pdfjs = await import("pdfjs-dist");
      pdfjs.GlobalWorkerOptions.workerSrc = new URL(
        "pdfjs-dist/build/pdf.worker.min.mjs",
        import.meta.url,
      ).toString();

      const pdf = await pdfjs.getDocument({
        data: await selectedFile.arrayBuffer(),
      }).promise;

      setFile(selectedFile);
      setTotalPages(pdf.numPages);
    } catch (caughtError) {
      console.error(caughtError);
      setError(
        "Não foi possível abrir este PDF. Verifique se o arquivo não está corrompido ou protegido por senha.",
      );
      event.target.value = "";
    }
  }

  function limpar() {
    setFile(null);
    setImages([]);
    setLoading(false);
    setTotalPages(null);
    setError(null);
  }

  async function converterPdf() {
    if (!file) return;

    setLoading(true);
    setError(null);
    setImages([]);

    try {
      const pdfjs = await import("pdfjs-dist");
      pdfjs.GlobalWorkerOptions.workerSrc = new URL(
        "pdfjs-dist/build/pdf.worker.min.mjs",
        import.meta.url,
      ).toString();

      const pdf = await pdfjs.getDocument({
        data: await file.arrayBuffer(),
      }).promise;

      const novasImagens: string[] = [];

      for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber++) {
        const page = await pdf.getPage(pageNumber);
        const viewport = page.getViewport({ scale: 2 });
        const canvas = document.createElement("canvas");
        const context = canvas.getContext("2d");

        if (!context) throw new Error("Canvas não suportado.");

        canvas.width = viewport.width;
        canvas.height = viewport.height;

        await page.render({
          canvas,
          canvasContext: context,
          viewport,
        }).promise;

        const image =
          format === "png"
            ? canvas.toDataURL("image/png")
            : canvas.toDataURL("image/jpeg", 0.95);

        novasImagens.push(image);
      }

      setImages(novasImagens);
    } catch (caughtError) {
      console.error(caughtError);
      setError(
        "Não foi possível converter este PDF. Tente outro arquivo ou verifique se o documento está protegido ou danificado.",
      );
    } finally {
      setLoading(false);
    }
  }

  async function baixarTodas() {
    if (!images.length) return;

    const zip = new JSZip();

    for (let i = 0; i < images.length; i++) {
      const response = await fetch(images[i]);
      const blob = await response.blob();
      zip.file(`pagina-${i + 1}.${format}`, blob);
    }

    const zipBlob = await zip.generateAsync({ type: "blob" });
    const url = URL.createObjectURL(zipBlob);
    const link = document.createElement("a");

    link.href = url;
    link.download = `${file?.name.replace(/\.pdf$/i, "") ?? "pdf"}-imagens.zip`;
    document.body.appendChild(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
  }

  return (
    <section className="min-h-screen bg-background text-foreground">
      <div className="mx-auto w-full max-w-6xl px-4 pb-12 pt-24 sm:px-6 lg:px-8 lg:pb-16">
        <div className="mb-8">
          <Link
            href="/ferramentas/pdfs"
            className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"
          >
            <ArrowLeft className="size-4" />
            Voltar para PDFs
          </Link>
        </div>

        <div className="mb-10 max-w-3xl">
          <p className="text-sm font-medium uppercase tracking-wider text-primary">DOCUMENTOS</p>
          <h1 className="mt-3 font-heading text-3xl font-semibold sm:text-4xl lg:text-5xl">
            PDF para Imagens
          </h1>
          <p className="mt-4 text-base leading-7 text-muted-foreground sm:text-lg">
            Converta cada página de um PDF em uma imagem PNG ou JPG.
          </p>
        </div>

        <Card className="mx-auto max-w-5xl">
          <CardHeader>
            <CardTitle>Selecione um PDF</CardTitle>
            <CardDescription>Escolha um arquivo PDF para converter todas as páginas em imagens.</CardDescription>
          </CardHeader>

          <CardContent>
            <label className="flex cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-primary/40 bg-muted/20 p-10 text-center transition hover:bg-muted/30">
              <Upload className="mb-4 size-8 text-primary" />
              <span className="font-medium">Clique para selecionar um PDF</span>
              <span className="mt-2 text-sm text-muted-foreground">Arquivos .pdf</span>
              <input
                type="file"
                accept="application/pdf,.pdf"
                className="hidden"
                onChange={handleFile}
              />
            </label>

            {error ? (
              <div
                role="alert"
                className="mt-5 rounded-lg border border-destructive/30 bg-destructive/10 p-4 text-sm leading-6 text-destructive"
              >
                {error}
              </div>
            ) : null}

            {file ? (
              <>
                <div className="mt-8 rounded-lg border p-4">
                  <div className="flex items-center gap-3">
                    <FileText className="size-5 text-primary" />
                    <div>
                      <p className="break-all font-medium">{file.name}</p>
                      <p className="text-sm text-muted-foreground">
                        {(file.size / 1024 / 1024).toFixed(2)} MB
                      </p>
                      {totalPages !== null ? (
                        <p className="text-sm text-muted-foreground">
                          {totalPages} página{totalPages > 1 ? "s" : ""}
                        </p>
                      ) : null}
                      <p className="text-sm text-muted-foreground">Saída: {format.toUpperCase()}</p>
                    </div>
                  </div>
                </div>

                <div className="mt-8">
                  <h3 className="mb-4 font-semibold">Formato de saída</h3>
                  <div className="flex gap-3">
                    <Button
                      variant={format === "png" ? "default" : "outline"}
                      onClick={() => changeFormat("png")}
                    >
                      PNG
                    </Button>
                    <Button
                      variant={format === "jpg" ? "default" : "outline"}
                      onClick={() => changeFormat("jpg")}
                    >
                      JPG
                    </Button>
                  </div>
                </div>

                <div className="mt-8 flex flex-wrap gap-3">
                  <Button onClick={converterPdf} disabled={loading}>
                    <FileImage className="size-4" />
                    {loading ? "Convertendo..." : "Converter"}
                  </Button>
                  <Button variant="outline" onClick={limpar} disabled={loading}>
                    <RotateCcw className="size-4" />
                    Limpar
                  </Button>
                  {images.length > 0 ? (
                    <Button variant="secondary" onClick={baixarTodas} disabled={loading}>
                      <Download className="size-4" />
                      Baixar todas em ZIP
                    </Button>
                  ) : null}
                </div>
              </>
            ) : null}
          </CardContent>
        </Card>

        {images.length > 0 ? (
          <div className="mt-10">
            <h3 className="mb-1 text-lg font-semibold">Pré-visualização</h3>
            <p className="mb-5 text-sm text-muted-foreground">
              {images.length} imagem{images.length > 1 ? "s" : ""} gerada{images.length > 1 ? "s" : ""} em {format.toUpperCase()}
            </p>

            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {images.map((image, index) => (
                <div key={index} className="rounded-xl border bg-card p-3 shadow-sm">
                  <h4 className="mb-3 text-sm font-semibold">
                    Página {index + 1} de {images.length}
                  </h4>
                  <img
                    src={image}
                    alt={`Página ${index + 1}`}
                    className="h-80 w-full rounded border bg-white object-contain"
                  />
                  <Button asChild className="mt-3 w-full">
                    <a href={image} download={`pagina-${index + 1}.${format}`}>
                      Baixar {format.toUpperCase()}
                    </a>
                  </Button>
                </div>
              ))}
            </div>
          </div>
        ) : null}

        <div className="mx-auto mt-8 max-w-5xl">
          <AdSlot variant="banner" />
        </div>
      </div>
    </section>
  );
}
