...
export default function PdfParaHtmlClient() {
  ...
  return <ToolPageShell title="Converter PDF para HTML" description="Transforme textos e conteúdos de um arquivo PDF em uma página HTML." categoryName="PDF" categoryHref="/ferramentas/pdfs" breadcrumbRootName="Início" breadcrumbRootHref="/" privacyMessage="Use somente documentos que você tem autorização para processar.">
    <Card className="mx-auto max-w-5xl"><CardHeader><CardTitle>Converter PDF para HTML</CardTitle><CardDescription>Extraia textos e estruturas para visualizar, editar ou reutilizar na web.</CardDescription></CardHeader><CardContent className="space-y-6">
      ...
      {result && <><ToolResultCard title="HTML pronto" description={`${result.name} · ${result.pages} ${result.pages === 1 ? "página convertida" : "páginas convertidas"} · ${formatFileSize(new Blob([safeHtml()]).size)}`} actions={<><Button onClick={download}><Download className="size-4" />Baixar HTML</Button><Button variant="outline" onClick={() => void copy()}><Clipboard className="size-4" />{copied ? "HTML copiado" : "Copiar HTML"}</Button><Button variant="outline" onClick={reset}><RotateCcw className="size-4" />Converter outro PDF</Button></>} />
        {result.scannedWarning && <p role="status" className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-4 text-sm">Este documento parece conter páginas digitalizadas como imagem. Parte do conteúdo pode exigir reconhecimento de texto.</p>}
