...
const AUDIO_FORMATS = [
  { mimeType: "audio/mp4;codecs=mp4a.40.2", extension: "m4a", label: "Áudio M4A" },
  { mimeType: "audio/mp4", extension: "m4a", label: "Áudio M4A" },
  { mimeType: "audio/webm;codecs=opus", extension: "webm", label: "Áudio WebM" },
  { mimeType: "audio/webm", extension: "webm", label: "Áudio WebM" },
];
...
export function VideoWorkbenchClient({ mode }: { mode: WorkbenchMode }) {
  ...
  const labels: Record<WorkbenchMode, { title: string; description: string; action: string }> = {
    ...
    audio: { title: "Vídeo para Áudio", description: "Extraia a trilha de áudio no formato compatível com seu navegador, sem enviar o vídeo para servidores.", action: "Extrair áudio" },
  };
  ...
  async function recordSegment(start: number, end: number, output: Dimensions, audio: boolean, suffix: string) {
    ...
    return { blob, url: URL.createObjectURL(blob), label: suffix, extension: "webm" };
  }

  async function extractAudio() {
    const video = videoRef.current;
    if (!video) throw new Error("Aguarde o vídeo carregar antes de processar.");
    if (!window.MediaRecorder) throw new Error("Este navegador não oferece gravação de áudio compatível.");
    const format = AUDIO_FORMATS.find((item) => MediaRecorder.isTypeSupported(item.mimeType));
    if (!format) throw new Error("Este navegador não oferece codificação de áudio compatível.");

    const capture = (video as HTMLVideoElement & { captureStream?: () => MediaStream; mozCaptureStream?: () => MediaStream }).captureStream?.()
      ?? (video as HTMLVideoElement & { mozCaptureStream?: () => MediaStream }).mozCaptureStream?.();
    const capturedTrack = capture?.getAudioTracks()[0];
    let stream: MediaStream;
    let disconnectAudio: (() => void) | undefined;

    if (capturedTrack) {
      stream = new MediaStream([capturedTrack]);
      video.muted = true;
    } else {
      const AudioContextConstructor = window.AudioContext ?? (window as typeof window & { webkitAudioContext?: typeof AudioContext }).webkitAudioContext;
      if (!AudioContextConstructor) throw new Error("Este navegador não consegue acessar a faixa de áudio deste vídeo.");
      ...
      await audioContext.resume();
      if (!destination.stream.getAudioTracks().length) throw new Error("Não foi encontrada uma faixa de áudio compatível neste vídeo.");
      stream = destination.stream;
      video.muted = false;
      disconnectAudio = () => audioSource.disconnect(destination);
    }

    const recorder = new MediaRecorder(stream, { mimeType: format.mimeType, audioBitsPerSecond: 192_000 });
    const chunks: BlobPart[] = [];
    recorder.ondataavailable = (event) => { if (event.data.size) chunks.push(event.data); };
    const stopped = new Promise<Blob>((resolve, reject) => {
      recorder.onerror = () => reject(new Error("Não foi possível gravar a faixa de áudio."));
      recorder.onstop = () => resolve(new Blob(chunks, { type: format.mimeType }));
    });
    const updateProgress = () => setProgress(Math.min(100, video.currentTime / Math.max(duration, 0.01) * 100));
    ...
    if (!blob.size) throw new Error("A faixa de áudio gerada ficou vazia. Tente outro vídeo.");
    return { blob, url: URL.createObjectURL(blob), label: format.label, extension: format.extension };
  }
  ...
  function download(result: Result, index: number) {
    if (!source) return;
    const link = document.createElement("a"); link.href = result.url; link.download = `${fileBaseName(source.file)}-${mode}${results.length > 1 ? `-parte-${index + 1}` : ""}.${result.extension}`; link.click();
  }
  ...
  return <main className="min-h-screen bg-background text-foreground"><div className="mx-auto w-full max-w-6xl px-4 pb-12 pt-24 sm:px-6 lg:pb-16 lg:px-8">
    ...
    {!source ? <div onDragEnter={(event: DragEvent<HTMLDivElement>) => { event.preventDefault(); setIsDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setIsDragging(false)} onDrop={(event) => { event.preventDefault(); setIsDragging(false); selectFile(event.dataTransfer.files?.[0]); }} className={`flex min-h-[400px] flex-col items-center justify-center rounded-xl border border-dashed p-6 text-center ${isDragging ? "border-primary bg-primary/10" : "border-border bg-muted/20"}`}><div className="flex size-14 items-center justify-center border border-border bg-background"><Upload className="size-5"/></div><p className="mt-6 text-xl font-semibold">Envie seu vídeo</p><p className="mt-3 max-w-md text-sm leading-6 text-muted-foreground">Arraste e solte o arquivo nesta área ou selecione um vídeo diretamente do seu dispositivo.</p><Button asChild className="mt-6"><label htmlFor="video-workbench-file">Selecionar vídeo</label></Button></div> : <div className="grid gap-6 lg:grid-cols-2"><div className="space-y-4"><div className="overflow-hidden rounded-xl border border-border bg-black"><video ref={videoRef} src={source.url} controls playsInline className="aspect-video w-full" onLoadedMetadata={onLoadedMetadata} onError={() => setError("Não foi possível abrir este vídeo no navegador.")}/></div><p className="text-sm text-muted-foreground">{source.file.name}{dimensions ? ` · ${dimensions.width} × ${dimensions.height}px · ${formatTime(duration)}` : ""}</p></div><div className="space-y-5 rounded-xl border border-border bg-muted/20 p-4 sm:p-5">
      ...
      {mode === "audio" && <p className="text-sm leading-6 text-muted-foreground">No iPhone e Safari, a saída é M4A quando disponível. Em Chrome, Edge e Firefox, a ferramenta também pode gerar WebM com Opus.</p>}
