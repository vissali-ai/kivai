...
import { Bot, BookOpen, CreditCard, FolderTree, Images, LayoutDashboard, LogOut, Megaphone, Plus, PanelsTopLeft, UsersRound, Wrench } from "lucide-react";
...
const originPanels = new Set(["/admin/blog", "/admin/blog/midias", "/admin/blog/categorias", "/admin/blog/agente", "/admin/site", "/admin/assinaturas", "/admin/usuarios", "/admin/marketing", "/admin/blog/manutencao"]);
  ...
  useEffect(() => {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (shouldReturn && response.ok) {
        ...
        if (origin && origin.startsWith("/admin/") && origin !== current) window.setTimeout(() => { router.replace(origin); router.refresh(); }, 150);
      }
      return response;
    };
