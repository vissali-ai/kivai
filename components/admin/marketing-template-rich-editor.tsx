...
export function MarketingTemplateRichEditor({
  initial,
  automatic,
  users = [],
  saveAction,
  ...
  const previewHtml = useMemo(
    () => ({ __html: value.message || "<p style='color:#7f8292'>Escreva o conteúdo para visualizar o e-mail.</p>" }),
    [value.message],
  );
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(420px,.9fr)]">
    <div className="space-y-5">
      <form action={saveAction} className="space-y-5 border border-white/10 bg-card p-5 sm:p-6">
        <label className="grid gap-1.5 text-sm"><span>Assunto</span><input name="subject" required maxLength={180} value={value.subject} onChange={(e) => setValue((current) => ({ ...current, subject: e.target.value }))} className="h-10 border border-white/10 bg-background px-3" /></label>
