...
export function InstagramFollowAnalyzerEditor({ initialConfig, initialContent, hubs }: { initialConfig: InstagramAnalyzerConfig; initialContent: ManagedSiteContent; hubs: SiteHub[] }) {
  ...
  function isDeleted(key: string) {
    return draft.deletedSections.includes(key);
  }

  function deleteSection(key: string, label: string) {
    if (!window.confirm(`Excluir definitivamente o bloco “${label}”? Os dados exclusivos deste bloco serão removidos e ele não poderá ser reativado pelo seletor.`)) return;
    setDraft((current) => {
      const next: InstagramAnalyzerConfig = {
        ...current,
        deletedSections: [...new Set([...current.deletedSections, key])],
        sectionVisibility: { ...current.sectionVisibility, [key]: false },
      };
      if (key === "hero") { next.eyebrow = ""; next.heroDescription = ""; next.badgeOne = ""; next.badgeTwo = ""; }
      if (key === "tutorial") { next.tutorialKicker = ""; next.tutorialTitle = ""; next.tutorialDescription = ""; next.metaButtonLabel = ""; next.metaUrl = ""; next.tutorialSteps = []; next.finalCta = ""; }
      if (key === "upload") { next.uploadTitle = ""; next.uploadDescription = ""; next.uploadLabel = ""; }
      if (key === "summaryPlans") { next.freeDescription = ""; next.proDescription = ""; next.agencyDescription = ""; }
      if (key === "audience") { next.audienceTitle = ""; next.audienceDescription = ""; }
      if (key === "plans") { next.plansTitle = ""; next.freePlanDetail = []; next.proPlanDetail = []; next.agencyPlanDetail = []; }
      if (key === "faq") { next.faqTitle = ""; next.faqItems = []; }
      if (key === "privacy") { next.privacyTitle = ""; next.privacyDescription = ""; next.privacyItems = []; next.privacyLinkLabel = ""; }
      return next;
    });
    setMessage(`Bloco “${label}” marcado para exclusão definitiva. Clique em Salvar alterações para gravar a remoção.`);
  }
  ...
  async function save() {
    ...
    try {
      ...
      setMessage("Alterações salvas. Blocos ocultos podem ser reativados; blocos excluídos tiveram os dados exclusivos removidos.");
    } catch (error) {
  ...
  return <main className="mx-auto max-w-6xl space-y-6">
    <header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Edição completa da ferramenta</p><h1 className="mt-1 text-2xl font-semibold sm:text-3xl">{draft.pageTitle}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Ocultar preserva o conteúdo. Excluir remove definitivamente os dados exclusivos daquele bloco da publicação.</p></div><div className="flex flex-wrap gap-2"><Button asChild type="button" variant="outline"><a href="/ferramentas/analisador-de-seguidores-instagram" target="_blank" rel="noreferrer"><ExternalLink />Abrir página</a></Button><Button type="button" onClick={save} disabled={busy}><Save />{busy ? "Salvando..." : "Salvar alterações"}</Button></div></header>
