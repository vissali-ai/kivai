...
export function InstagramFollowAnalyzerEditor({ initialConfig, initialContent, hubs }: { initialConfig: InstagramAnalyzerConfig; initialContent: ManagedSiteContent; hubs: SiteHub[] }) {
  ...
  return <main className="mx-auto max-w-6xl space-y-6">
    <header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Edição completa da ferramenta</p><h1 className="mt-1 text-2xl font-semibold sm:text-3xl">{draft.pageTitle}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Ocultar preserva o conteúdo. Excluir remove definitivamente os dados exclusivos daquele bloco da publicação.</p></div><div className="flex flex-wrap gap-2"><Button asChild type="button" variant="outline"><a href="/ferramentas/instagram-follow-analyzer" target="_blank" rel="noreferrer"><ExternalLink />Abrir página</a></Button><Button type="button" onClick={save} disabled={busy}><Save />{busy ? "Salvando..." : "Salvar alterações"}</Button></div></header>
