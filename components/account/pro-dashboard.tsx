...
import { ArrowRight, BarChart3, CalendarClock, Crown, History, Instagram, LockKeyhole, RefreshCw, UsersRound } from "lucide-react";
...
export function ProDashboard() {
  ...
  return <div className="space-y-6">
    ...
    <section className="border border-white/10 bg-card"><div className="flex items-center justify-between border-b border-white/10 p-5"><div><h2 className="font-semibold">Perfis do Instagram</h2><p className="mt-1 text-xs text-muted-foreground">Cada nova exportação atualiza o histórico do perfil.</p></div><Instagram className="size-5 text-primary" /></div>{accounts.length ? <div className="divide-y divide-white/10">{accounts.map((account) => <div key={account.id} className="grid gap-3 p-5 sm:grid-cols-[1fr_auto_auto_auto] sm:items-center"><div><p className="font-medium">@{account.username}</p><p className="mt-1 text-xs text-muted-foreground">Última análise: {account.last_analyzed_at ? new Date(account.last_analyzed_at).toLocaleString("pt-BR") : "-"}</p></div><div className="text-sm"><span className="block text-xs text-muted-foreground">Seguidores</span>{account.follower_count?.toLocaleString("pt-BR") ?? "-"}</div><div className="text-sm"><span className="block text-xs text-muted-foreground">Seguindo</span>{account.following_count?.toLocaleString("pt-BR") ?? "-"}</div><Button asChild size="sm" variant="outline"><Link href="/ferramentas/instagram-follow-analyzer">Atualizar análise</Link></Button></div>)}</div> : <div className="p-8 text-center"><p className="text-sm text-muted-foreground">Você ainda não possui perfis salvos.</p><Button asChild className="mt-4"><Link href="/ferramentas/instagram-follow-analyzer">Fazer primeira análise</Link></Button></div>}</section>
