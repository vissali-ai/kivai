...
export function AccountDashboard() {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 md:grid-cols-2">
      <Link href="/ferramentas/instagram-follow-analyzer" className="group rounded-2xl border border-white/10 bg-card p-5 transition hover:border-primary/40"><div className="flex size-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.03] text-primary"><Sparkles className="size-5" /></div><h2 className="mt-4 font-semibold">Acessar Instagram Follow Analyzer</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Acesse a sua ferramenta de diagnóstico do Instagram e tenha acesso a uma nova análise.</p><span className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-primary">Acessar ferramenta <ArrowRight className="size-4" /></span></Link>
    ...
    <section className="rounded-3xl border border-primary/20 bg-primary/[0.035] p-6 sm:p-8"><div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Mais recursos</p><h2 className="mt-2 text-2xl font-semibold">Soluções para cada fase da sua presença digital</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Os planos pagos ampliam limites e recursos do Instagram Follow Analyzer, enquanto os serviços Kivai ajudam usuários, marcas, contas em crescimento e agências a fortalecer comunicação, divulgação e resultados digitais.</p></div><div className="grid gap-2"><Button asChild><Link href="/planos">Planos Instagram Follow Analyzer <ArrowRight /></Link></Button><Button asChild variant="outline"><Link href="/servicos">Nossos Serviços <ArrowRight /></Link></Button></div></div></section>
