...
export function AccountDashboard() {
  ...
  if (loading) return <div className="flex min-h-[55vh] items-center justify-center"><div className="text-center"><div className="mx-auto size-9 animate-pulse rounded-full border border-primary/30 bg-primary/10" /><p className="mt-4 text-sm text-muted-foreground">Carregando seu painel...</p></div></div>;
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-card p-6 sm:p-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            ...
            <div className="mt-5 flex flex-wrap gap-3"><Button asChild><Link href="/ferramentas/instagram-follow-analyzer">Abrir ferramenta <ArrowRight /></Link></Button><Button asChild variant="outline"><Link href="/"><Home /> Voltar para a Home</Link></Button></div>
          </div>
          <div className="min-w-[230px] rounded-2xl border border-primary/20 bg-primary/[0.05] p-5">
            ...
            {plan === "free" ? <Button asChild size="sm" className="mt-5 w-full"><Link href="/planos"><Sparkles /> Conhecer planos pagos</Link></Button> : <Button asChild size="sm" variant="outline" className="mt-5 w-full"><Link href="/planos"><RefreshCw /> Renovar plano</Link></Button>}
          </div>
        </div>
      </section>
        ...
        <section className="rounded-2xl border border-primary/25 bg-primary/[0.035] p-5 sm:p-6">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-muted-foreground">{pendingRequest.status === "payment_reported" ? "Pagamento informado. Aguardando o administrador conferir a confirmação da SumUp e ativar o plano." : "Depois de concluir o pagamento na SumUp, informe aqui para deixar a solicitação destacada no Admin."}</p>
            </div>
            <div className="flex min-w-[220px] flex-col gap-2">
              {pendingRequest.status === "awaiting_payment" ? <><Button onClick={reportPayment} disabled={reportingPayment}>{reportingPayment ? "Registrando..." : "Já realizei o pagamento"}</Button><Button asChild variant="outline"><a href={pendingRequest.payment_link} target="_blank" rel="noopener noreferrer">Abrir pagamento novamente</a></Button></> : <div className="flex items-center justify-center gap-2 border border-primary/30 bg-primary/10 px-4 py-3 text-sm font-medium text-primary"><CheckCircle2 className="size-4" /> Aguardando confirmação</div>}
            </div>
          </div>
        </section>
      ) : null}
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <Link href="/ferramentas/instagram-follow-analyzer" className="group rounded-2xl border border-white/10 bg-card p-5 transition hover:-translate-y-0.5 hover:border-primary/40"><div className="flex size-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.03] text-primary"><Sparkles className="size-5" /></div><h2 className="mt-4 font-semibold">Quem deixou de seguir</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Importe seus dados do Instagram e gere uma nova análise.</p><span className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-primary">Acessar ferramenta <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></span></Link>
