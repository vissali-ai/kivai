...
type PublishedPostCampaign = { id: string; title: string; slug: string; excerpt: string; publishedAt: string | null };

function render(value: string, post: PublishedPostCampaign) {
  return value.replaceAll("{{titulo}}", post.title).replaceAll("{{resumo}}", post.excerpt).replaceAll("{{slug}}", post.slug).replaceAll("{{link}}", `${SITE_URL}/blog/${post.slug}`);
}

export async function deliverPendingBlogPostEmails(limit = 50) {
  const rows = await supabaseRest<Array<{ id: string; status: string; metadata: Record<string, unknown> | null }>>(`customer_communications?select=id,status,metadata&channel=eq.email&status=in.(ready,failed)&metadata->>flow_key=eq.new_post&scheduled_for=lte.${encodeURIComponent(new Date().toISOString())}&order=created_at.asc&limit=${limit}`);
  ...
  for (const row of rows) {
    if (row.status === "failed") {
      ...
      if (retries >= 3) { results.skipped += 1; continue; }
      await supabaseRest(`customer_communications?id=eq.${encodeURIComponent(row.id)}&status=eq.failed`, { method: "PATCH", body: JSON.stringify({ status: "ready", error: null, metadata: { ...(row.metadata ?? {}), blog_retry_count: retries + 1 }, updated_at: new Date().toISOString() }) });
    }
    const result = await deliverCustomerEmail(row.id);
    if (result.status === "sent") results.sent += 1; else if (result.status === "canceled") results.canceled += 1; else if (result.status === "failed") results.failed += 1; else results.skipped += 1;
  }
  return results;
}
...
export async function queueNewPostCampaign(post: PublishedPostCampaign) {
  ...
  const rows = await supabaseRest<Array<{ id: string }>>("customer_communications?on_conflict=event_key,channel", {
    ...
    body: JSON.stringify(users.map((user) => ({
      ...
      cta_url: template.cta_url ? render(template.cta_url, post) : postUrl,
